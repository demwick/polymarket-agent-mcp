...
import { getConfig, getHttpAuthToken, hasLiveCredentials, validateLiveCredentials } from "./utils/config.js";
...
// A fresh MCP server per connection: the SDK's Protocol.connect() throws
// 'Already connected to a transport' if one instance is connected twice, so
// the stateless HTTP path needs its own instance for every request.
...
async function main() {
  ...
  if (useHttp) {
    await startHttpServer(config.PORT ?? 3000);
  } else {
...
export function startHttpServer(port: number): Promise<Server> {
  ...
  return new Promise((resolve) => {
    httpServer.listen(port, () => {
      ...
      log("info", `HTTP transport listening on port ${actualPort}`);
      log("info", `MCP endpoint: http://localhost:${actualPort}/mcp`);
      resolve(httpServer);
    });
  });
}
...
// Only boot when executed as a program; importing this module (tests) must not
// start a server or install signal handlers.
